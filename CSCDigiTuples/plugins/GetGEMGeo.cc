...
class GetGEMGeo : public edm::one::EDAnalyzer<> {

public:
  GetGEMGeo( const edm::ParameterSet& pset);
...
void
GetGEMGeo::analyze( const edm::Event& /*iEvent*/, const edm::EventSetup& iSetup )
{
  ...
  for (auto sch : ring->superChambers()) {
    ...
    for (auto ch : sch->chambers()){
      ...
      for (auto roll : rolls){
        ...
        ofos2 << k <<" " << parameters[0] <<" "<<parameters[1] <<" "<< parameters[2]<<" "<<parameters[3] <<endl;
