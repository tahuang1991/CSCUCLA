...
#include "BaseCSCAndGEMAnalyzer.h"
...
public:
  void bookHistos(HistGetter& plotter){
    ...
    for(unsigned int iV = 0; iV < GEMInfo::NVFAT + 1; ++iV){
      TString name = TString::Format("%u",iV);
      if(iV == GEMInfo::NVFAT) name = "all";
    ...
    plotter.book2D("numClusters_2D_hit",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
    plotter.book2D("numClusters_2D_all",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);

//    plotter.book2D("numClusters_signal_hit",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
//    plotter.book2D("numClusters_signal_all",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);

    plotter.book2D("numClusters_2D_noiseEvent_hit",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
    plotter.book2D("numClusters_2D_noise_hit",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
    plotter.book2D("numClusters_2D_noise_all",";Number of clusters"  ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
  }
  ...
  void fillHistos(Analyzer * ana, HistGetter& plotter){
    ...
    std::vector<int> vFatIdx(GEMInfo::NVFAT,-1);
    for(unsigned int iV = 0; iV < gem.vFats.size(); ++iV){vFatIdx[gem.vFats[iV].idx] = iV;}

    //Now fill number of clusters and size
    std::vector<int> nClus(GEMInfo::NVFAT,0);
    std::vector<std::vector<int> > clusSize(GEMInfo::NVFAT,std::vector<int>(0));
    getClustersByVFAT(gem,nClus,clusSize);

    //Now we find our "signal" vfats and
    std::vector<bool> isSignalVFat(GEMInfo::NVFAT,false);
    std::vector<bool> isNoiseVFat(GEMInfo::NVFAT,true);
    getSigNoise(ana,csc,gem,isSignalVFat,isNoiseVFat);



    plotter.get1D("numClusters_all")->Fill(gem.clusters.size());
    int nClusNoise = 0;
    for(unsigned int iC = 0; iC < gem.clusters.size(); ++iC){
      plotter.get1D("clusterSize_all")->Fill(gem.clusters[iC].getNStrips());
      if(isNoiseVFat[gem.clusters[iC].nRow] && pure) plotter.get1D("clusterSize_noise_all")->Fill(gem.clusters[iC].getNStrips());
      if(isSignalVFat[gem.clusters[iC].nRow] && pure) plotter.get1D("clusterSize_signal_all")->Fill(gem.clusters[iC].getNStrips());
    }
    ...
    for(unsigned int iV = 0; iV < GEMInfo::NVFAT; ++iV){
      ...
      if(iV >= 8) idx = 1;
      if(iV >= 16) idx = 0;
      if(nClus[iV]) plotter.get2D("numClusters_2D_hit")->Fill(idx,GEMInfo::getRow(iV),nClus[iV]);
      plotter.get2D("numClusters_2D_all")->Fill(idx,GEMInfo::getRow(iV));

      if(isNoiseVFat[iV] && pure){
        if(nClus[iV]) plotter.get2D("numClusters_2D_noise_hit")->Fill(idx,GEMInfo::getRow(iV),nClus[iV]);
        if(nClus[iV]) plotter.get2D("numClusters_2D_noiseEvent_hit")->Fill(idx,GEMInfo::getRow(iV));
        plotter.get2D("numClusters_2D_noise_all")->Fill(idx,GEMInfo::getRow(iV));
      }
    }
  ...
  void getSigNoise(Analyzer * ana, AnalyzeCSC&  csc, AnalyzeGEM&  gem, std::vector<bool>& isSignalVFat, std::vector<bool>& isNoiseVFat ){
    ...
    double spX = sX +gem.geo.ZDIST*csc.segmentInfo.segment_dxdz->at(0);
    double spY = sY +gem.geo.ZDIST*csc.segmentInfo.segment_dydz->at(0);

    const auto& geo = gem.geo;
    const auto& vfatgeo = gem.geo.vFat;
    auto isInVFat = [](double& in, const GEMInfo::VFATGeo& geo, bool strict) -> bool { return (geo.isValid)  && (in < geo.yMax) && ( strict ? in > geo.yMin : 1.0) ;};
...
//        }
  ...
  void getClustersByVFAT(AnalyzeGEM& gem,std::vector<int>& nClus, std::vector<std::vector<int> >& clusSize){
    for(unsigned int iC = 0; iC < gem.clusters.size(); ++iC){
      bool fill_col1 = false; //left to right
      bool fill_col2 = false;
      bool fill_col3 = false;

      if(gem.clusters[iC].getFirstStrip() >= 256  ){
        fill_col1 = true;
        if(gem.clusters[iC].getLastStrip() <= 255 ) fill_col2 = true;
        if(gem.clusters[iC].getLastStrip() <= 127 ) fill_col3 = true;
      } else if (gem.clusters[iC].getFirstStrip() >= 128  ) {
        fill_col2 = true;
        if(gem.clusters[iC].getLastStrip() <= 127 ) fill_col3 = true;
      } else{
        fill_col3 = true;
      ...
      if(fill_col1) fillVFAT.push_back(gem.clusters[iC].nRow +16);
      if(fill_col2) fillVFAT.push_back(gem.clusters[iC].nRow + 8);
      if(fill_col3) fillVFAT.push_back(gem.clusters[iC].nRow);

      for(unsigned int iV = 0; iV < fillVFAT.size(); ++iV){
        nClus[fillVFAT[iV]]++;
        clusSize[fillVFAT[iV]].push_back(gem.clusters[iC].getNStrips());
      }
...
public:
  void bookHistos(HistGetter& plotter){
    ...
    plotter.book2D("VFATOcc"                   ,";VFAT #"          ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
    plotter.book2D("VFATOcc_all"                   ,";VFAT #"          ,GEMInfo::NCOLS, -0.5, GEMInfo::NCOLS -.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);

    plotter.book2D("StripMap"    ,";Strip #;VFAT #",130,-0.5,129.5,GEMInfo::NVFAT,-0.5,GEMInfo::NVFAT - 0.5 );
    plotter.book2D("StripMapPhysical"    ,"",384,-0.5,383.5,GEMInfo::NROWS, -0.5, GEMInfo::NROWS -.5);
  ...
  void fillHistos(Analyzer * ana, HistGetter& plotter){
    ...
    for(unsigned int iV = 0; iV < GEMInfo::NVFAT; ++iV){
      ...
      if(iV >= 8) idx = 1;
      if(iV  >= 16) idx = 0;
      plotter.get2D("VFATOcc_all")->Fill(idx,GEMInfo::getRow(iV));

    }

    for(unsigned int iV = 0; iV < gem.vFats.size(); ++iV){
      auto& vFat = gem.vFats[iV];
      int idx=2;
      if(vFat.idx >= 8) idx = 1;
      ...
      for(unsigned int iS = 0; iS < vFat.nStrips(); ++iS){
        ...
        plotter.get2D("StripMapPhysical")->Fill(GEMInfo::getGlobalStripXLeftIs0(vFat.idx,vFat.strips[iS]),vFat.nRow);
      }
    }
    ...
    if(pure){
      ...
      double spX = sX +gem.geo.ZDIST*csc.segmentInfo.segment_dxdz->at(iS);
      double spY = sY +gem.geo.ZDIST*csc.segmentInfo.segment_dydz->at(iS);
...
public:
  ...
  void fillHistos(Analyzer * ana, HistGetter& plotter){
    ...
    if(csc.wireInfo.wire_bx->at(0) - gem.BX > 0)
      plotter.get1D("BXDiff")->Fill(csc.wireInfo.wire_bx->at(0) - gem.BX);
    else
      plotter.get1D("BXDiff")->Fill(  csc.wireInfo.wire_bx->at(0) - gem.BX + 3564 );

    plotter.get1D("nEvents")->Fill(0);
    if(csc.segmentInfo.segment_pos_x->size()) plotter.get1D("nEvents")->Fill(1);
    if(gem.vFats.size())plotter.get1D("nEvents")->Fill(2);
    if(csc.segmentInfo.segment_pos_x->size() && gem.vFats.size())plotter.get1D("nEvents")->Fill(3);
  }
